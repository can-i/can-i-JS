...
export abstract class BaseControllerDecoratorBuilder extends DecoratorBuilder {
    ...
    protected getControllerOptions<T extends MainController>(c: { new (): T }): ControllerOptions<T> {
        return GetControllerOptions().filter(o => {
            return o.klass === c
        }).reduce((prev, next) => prev || next, null as any) || new ControllerOptions(c);
    }

    protected getMethodOption<T extends MainController>(constructor: { new (): T }, name: string): ControllerMethodOptions<T> {
